  ...
  <div>
    ...
    <el-table-column
	  ...
	  <template slot-scope="scope">
		  ...
		  <span @click="shipOrders1(scope.row.id)" style="margin-right:10px;color:#34c38f;cursor: pointer;"  v-if="scope.row.status == 'Shipping'">Đang giao</span>
	...
	methods:{
		listOrders(){
			this.$store.dispatch('orders/listOrders',
		 	 ...
		 	 shipOrders1(id){
		 	this.$store.dispatch('orders/shipOrders1',id).then((res)=>{
				  this.open2()
			 })
			 console.log('idd',id)
		 },
